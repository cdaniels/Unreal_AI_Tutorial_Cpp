...
{
    // attempt to get behavior tree from reference in UE4Editor
    static ConstructorHelpers::FObjectFinder<UBehaviorTree> obj(TEXT("BehaviorTree'/Game/AI_Tutorial/AI/NPC_BT.NPC_BT'"));

    if (obj.Succeeded())
    {
        btree = obj.Object;
    }
    // create a behaviortree and blackboard component
    btree_component = object_initializer.CreateDefaultSubobject<UBehaviorTreeComponent>(this, TEXT("BehaviorComponent"));
    blackboard = object_initializer.CreateDefaultSubobject<UBlackboardComponent>(this, TEXT("BlackboardComp"));

    // setup the ai perception
    setup_perception_system();
}

void ANPC_AIController::BeginPlay()
{
    // set the controller to run its behavior tree
    Super::BeginPlay();
...
void ANPC_AIController::OnPossess(APawn* const pawn)
{
    ...
    // initialize the blackboard so its boand to the behavior tree
    if(blackboard)
    {
...
{
    if (auto const ch = Cast<AAI_Tutorial_CppCharacter>(actor))
    {
